...
locals {
  prefix_regression_test                                = "regression-test-"
  prefix_regression_test_dir                            = "${local.prefix_regression_test}dir-"
  prefix_regression_test_account                        = "${local.prefix_regression_test}acc-"
  regression_test_account_static                        = "${local.prefix_regression_test_account}static"
  regression_test_account_static_extended               = "${local.regression_test_account_static}-${var.subaccount_subdomain_extension}"
  regression_test_account_entitlements_stacked          = "${local.prefix_regression_test_account}entitlements-stacked"
  regression_test_account_entitlements_stacked_extended = "${local.regression_test_account_entitlements_stacked}-${var.subaccount_subdomain_extension}"
  ...
  regression_test_security_settings                     = "${local.prefix_regression_test}security-settings"
  regression_test_security_settings_extended            = "${local.regression_test_security_settings}-${var.subaccount_subdomain_extension}"
  regression_test_dir_static                            = "${local.prefix_regression_test_dir}static"
  regression_test_dir_se_static                         = "${local.prefix_regression_test_dir}se-static"
  regression_test_dir_entitlements                      = "${local.prefix_regression_test_dir}entitlements"
  regression_test_dir_entitlements_stacked              = "${local.prefix_regression_test_dir}entitlements-stacked"
  disclaimer_description                                = "Please don't modify. This is used for regression tests."
}
  ...
  roles = [
    {
      ...
      role_template_app_id = var.globalaccount_role_template_app_id
...
resource "btp_directory" "dir_entitlements" {
  name     = local.regression_test_dir_entitlements
...
resource "btp_directory" "dir_entitlements_stacked" {
  ...
  name      = local.regression_test_dir_entitlements_stacked
}

resource "btp_directory" "dir_static" {
  name        = local.regression_test_dir_static
  features    = ["DEFAULT", "ENTITLEMENTS", "AUTHORIZATIONS"]
  description = local.disclaimer_description
}

resource "btp_directory" "dir_se_static" {
  name        = local.regression_test_dir_se_static
  description = local.disclaimer_description
  features    = ["DEFAULT", "ENTITLEMENTS", "AUTHORIZATIONS"]
  labels = {
    my-label-1 = [
...
resource "btp_directory_role_collection" "drc_dir_static" {
  ...
  name         = "${local.prefix_regression_test_dir}static-role-collection"
  description  = local.disclaimer_description

  roles = [
    {
      name                 = "Directory Admin"
      role_template_app_id = var.directory_role_template_app_id
...
resource "btp_subaccount" "sa_acc_static" {
  name        = local.regression_test_account_static
  description = local.disclaimer_description
  subdomain   = local.regression_test_account_static_extended
  region      = var.region
  labels = {
    label1 = [
      "label text 1"
...
resource "btp_subaccount" "sa_acc_entitlements_stacked" {
  ...
  name      = local.regression_test_account_entitlements_stacked
  subdomain = local.regression_test_account_entitlements_stacked_extended
  region    = var.region
}
...
resource "btp_subaccount" "sa_security_settings" {
  name      = local.regression_test_security_settings
  subdomain = local.regression_test_security_settings_extended
  region    = var.region
}
  ...
  roles = [
    {
      ...
      role_template_app_id = var.subaccount_role_template_app_id
