...
The environment variables recognized by the provider are `BTP_USERNAME`, `BTP_PASSWORD`, `BTP_IDTOKEN`, `BTP_ASSERTION`, and `BTP_IDP`. The `tls_client_*` attributes have no environment-variable fallback and must be set explicitly.
...
1. Set the environment variable `USE_BTPCLI_SESSION` to `true`
2. Login via btp CLI e.g, via `btp login --sso`
3. Optional: if you have a special path to your btp CLI configuration, set it via environment variable `BTPCLI_CONFIG_PATH`
4. Configure the provider with the subdomain of the global account subdomain
5. Execute the terraform command e.g. `terraform plan` or `terraform apply`

You will not be prompted for additional credentials if the btp CLI session is active and valid. The provider will use the session information from the btp CLI to authenticate to SAP BTP and execute the terraform commands.
