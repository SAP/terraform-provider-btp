...
3. Optional: if you have a special path to your btp CLI configuration, set it via environment variable `PATH_TO_CLI_CONFIG`
...
You will no be prompted for additional credentials if the btp CLI session is active and valid. The provider will use the session information from the btp CLI to authenticate to SAP BTP and execute the terraform commands.

**Prerequiste**: You need to have the btp CLI installed, be logged in to the btp CLI, and have a valid session. You can log in to the btp CLI using the command `btp login --sso`. For more information see the [official documentation of the btp CLI](https://help.sap.com/docs/btp/sap-business-technology-platform/account-administration-using-sap-btp-command-line-interface-btp-cli).
