...
list "btp_subaccount_trust_configuration" "<label_name>" {
...
# List block to discover all trust configurations for given subaccount
# Returns only the resource identities by default.
list "btp_subaccount_trust_configuration" "all" {
  provider = btp
...
# List block to discover all trust configurations for given subaccount with full resource details
# Setting include_resource = true returns full resource objects (e.g., platform_id, name..)
list "btp_subaccount_trust_configuration" "with_resource" {
  provider         = btp
