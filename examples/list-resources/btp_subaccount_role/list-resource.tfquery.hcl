...
list "btp_subaccount_role" "<label_name>" {
...
# List block to discover all subaccount roles for given subaccount
# Returns only the resource identities by default.
list "btp_subaccount_role" "all" {
  provider = btp
...
# List block to discover all subaccount roles with full resource details
# Setting include_resource = true returns full resource objects (e.g., platform_id, name..)
list "btp_subaccount_role" "with_resource" {
  provider         = btp
