...
list "btp_globalaccount_role_collection" "<label_name>" {
  # (Required) Provider instance to use
  provider = provider_name
}

# List block to discover all role collections in global account
# Returns only the resource identities (names) by default.
list "btp_globalaccount_role_collection" "all" {
  provider = btp
}

# List block to discover all role collections in global account with full resource details
# Setting include_resource = true returns full resource objects (e.g., description,name..)
list "btp_globalaccount_role_collection" "with_resource" {
  provider         = btp
