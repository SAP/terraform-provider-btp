...
func TestAccountsSubaccountFacade_Restore(t *testing.T) {
	command := "accounts/subaccount"
	...
	t.Run("constructs the CLI params correctly", func(t *testing.T) {
		...
		uut, srv := prepareClientFacadeForTest(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			assertCall(t, r, command, ActionRestore, map[string]string{
				"subaccount": subaccountId,
			})
		...
		_, res, err := uut.Accounts.Subaccount.Restore(context.TODO(), subaccountId)
