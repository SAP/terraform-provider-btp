...
func newSubaccountAppDataSource() datasource.DataSource {
	return &subaccountAppDataSource{}
}

type subaccountAppDataSourceConfig struct {
	/* INPUT */
...
type subaccountAppDataSource struct {
	cli *btpcli.ClientFacade
}

func (ds *subaccountAppDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = fmt.Sprintf("%s_subaccount_app", req.ProviderTypeName)
}

func (ds *subaccountAppDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
...
func (ds *subaccountAppDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data subaccountAppDataSourceConfig
	...
	cliRes, _, err := ds.cli.Security.App.GetBySubaccount(ctx, data.SubaccountId.ValueString(), data.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("API Error Reading Resource App (Subaccount)", fmt.Sprintf("%s", err))
		return
	}
