...
func newGlobalaccountAppDataSource() datasource.DataSource {
	return &globalaccountAppDataSource{}
}

type globalaccountAppOauthConfiguration struct {
	Allowedproviders     types.Set   `tfsdk:"allowedproviders"`
	Autoapprove          types.Bool  `tfsdk:"autoapprove"`
	GrantTypes           types.Set   `tfsdk:"grant_types"`
	RedirectUris         types.Set   `tfsdk:"redirect_uris"`
	RefreshTokenValidity types.Int64 `tfsdk:"refresh_token_validity"`
	SystemAttributes     types.Set   `tfsdk:"system_attributes"`
	TokenValidity        types.Int64 `tfsdk:"token_validity"`
}

type globalaccountAppDataSourceConfig struct {
	/* INPUT */
...
type globalaccountAppDataSource struct {
	cli *btpcli.ClientFacade
}

func (ds *globalaccountAppDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = fmt.Sprintf("%s_globalaccount_app", req.ProviderTypeName)
}

func (ds *globalaccountAppDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
...
func (ds *globalaccountAppDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (ds *globalaccountAppDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data globalaccountAppDataSourceConfig
	...
	cliRes, _, err := ds.cli.Security.App.GetByGlobalAccount(ctx, data.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("API Error Reading Resource App (Global Account)", fmt.Sprintf("%s", err))
		return
	}
