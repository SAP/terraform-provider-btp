...
func (ds *subaccountSubscriptionDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: `Get the details of a specific multitenant application to which a subaccount is entitled to subscribe. If this application is in a different global account than the current one, you need to specify its plan with '--plan'.
...
You must be assigned to the subaccount admin or viewer role.`,
		Attributes: map[string]schema.Attribute{
			...
			"customer_developed": schema.BoolAttribute{
				MarkdownDescription: "Whether the application was developed by a customer. If not, then the application is developed by the cloud operator, such as SAP.",
				Computed:            true,
			},
			...
			"formation_solution_name": schema.StringAttribute{
				MarkdownDescription: "Name of the formations solution associated with the multitenant application.",
				Computed:            true,
			},
			"globalaccount_id": schema.StringAttribute{
				MarkdownDescription: "ID of the associated global account.",
				Computed:            true,
			},
			...
			"platform_entity_id": schema.StringAttribute{
				MarkdownDescription: "ID of the landscape-specific environment.",
				Computed:            true,
			},
			"quota": schema.Int64Attribute{
				MarkdownDescription: "Total amount the subscribed subaccount is entitled to consume.",
				Computed:            true,
			},
			...
			"id": schema.StringAttribute{
				MarkdownDescription: "Technical ID generated by XSUAA for a multitenant application when a consumer subscribes to the application.",
				Computed:            true,
			},
			"subscription_url": schema.StringAttribute{
				MarkdownDescription: "URL for app users to launch the subscribed application.",
				Computed:            true,
			},
			...
			"supports_plan_updates": schema.BoolAttribute{
				MarkdownDescription: "Specifies if a consumer, whose subaccount is subscribed to the application, can change the subscription to a different plan that is available for this application and subaccount.",
				Computed:            true,
			},
			"tenant_id": schema.StringAttribute{
				MarkdownDescription: "Tenant ID of the application provider.",
				Computed:            true,
			},
