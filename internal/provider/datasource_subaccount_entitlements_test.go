...
func TestDataSourceSubaccountEntitlements(t *testing.T) {
			...
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeAggregateTestCheckFunc(
						...
						resource.TestCheckResourceAttr("data.btp_subaccount_entitlements.uut", "values.%", "31"),
					),
				},
			},
		})
	...
	t.Run("error path - subaccount_id not a valid UUID", func(t *testing.T) {
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config:      hclDatasourceSubaccountEntitlementsBySubaccountId("uut", "this-is-not-a-uuid"),
