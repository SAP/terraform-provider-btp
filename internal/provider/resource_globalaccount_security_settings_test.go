	...
	t.Run("happy path - complete configuration", func(t *testing.T) {
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: hclProviderFor(user) + hclResourceGlobalaccountSecuritySettings("uut", "terraformint.accounts400.ondemand.com", 3601, 3602, true, "[\"domain1.test\",\"domain2.test\"]"),
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("btp_globalaccount_security_settings.uut", "access_token_validity", "3601"),
						resource.TestCheckResourceAttr("btp_globalaccount_security_settings.uut", "refresh_token_validity", "3602"),
