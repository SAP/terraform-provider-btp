...
func newGlobalaccountEntitlementsWithDcDataSource() datasource.DataSource {
	return &globalaccountEntitlementsWithDcDataSource{}
}

type entitledServiceWithDc struct {
...
func entitledServiceWithDcType() map[string]attr.Type {
	return map[string]attr.Type{
		...
		"datacenter_information": types.MapType{ElemType: types.ObjectType{AttrTypes: datacenterInformationType()}},
	}
}

type datacenterInformation struct {
	Region       types.String `tfsdk:"dc_region"`
	Name         types.String `tfsdk:"dc_name"`
	DisplayName  types.String `tfsdk:"dc_display_name"`
	IaasProvider types.String `tfsdk:"dc_iaas_provider"`
}

func datacenterInformationType() map[string]attr.Type {
	return map[string]attr.Type{
		"dc_region":        types.StringType,
		"dc_name":          types.StringType,
		"dc_display_name":  types.StringType,
		"dc_iaas_provider": types.StringType,
	}
}

type globalaccountEntitlementsWithDcDataSourceConfig struct {
	/* INPUT */
...
type globalaccountEntitlementsWithDcDataSource struct {
	cli *btpcli.ClientFacade
}

func (ds *globalaccountEntitlementsWithDcDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = fmt.Sprintf("%s_globalaccount_entitlements_with_data_centers", req.ProviderTypeName)
}

func (ds *globalaccountEntitlementsWithDcDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
...
func (ds *globalaccountEntitlementsWithDcDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: `Gets all the entitlements and quota assignments for a global account including the data center availability.
...
func (ds *globalaccountEntitlementsWithDcDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data globalaccountEntitlementsWithDcDataSourceConfig
	...
	values := map[string]entitledServiceWithDc{}
	...
	for _, service := range cliRes.EntitledServices {
		for _, servicePlan := range service.ServicePlans {
			...
			values[fmt.Sprintf("%s:%s", service.Name, servicePlan.Name)] = entitledServiceWithDc{
	...
	data.Values, diags = types.MapValueFrom(ctx, types.ObjectType{AttrTypes: entitledServiceWithDcType()}, values)
	resp.Diagnostics.Append(diags...)
