...
func newGlobalaccountAppsDataSource() datasource.DataSource {
	return &globalaccountAppsDataSource{}
}

type globalAccountAppsValue struct {
...
type globalaccountAppsDataSourceConfig struct {
	/* INPUT */
	/* OUTPUT */
	Values []globalAccountAppsValue `tfsdk:"values"`
}

type globalaccountAppsDataSource struct {
	cli *btpcli.ClientFacade
}

func (ds *globalaccountAppsDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = fmt.Sprintf("%s_globalaccount_apps", req.ProviderTypeName)
}

func (ds *globalaccountAppsDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
...
func (ds *globalaccountAppsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data globalaccountAppsDataSourceConfig
	...
	cliRes, _, err := ds.cli.Security.App.ListByGlobalAccount(ctx)
	if err != nil {
		resp.Diagnostics.AddError("API Error Reading Resource Apps (Global Account)", fmt.Sprintf("%s", err))
		return
	}
